...
# Official Jira set recommendation

The assignment suggests approximately 10-15 user stories. This document contains 16 because the libGDX presentation layer is useful for final presentation and must be explicitly separated from the core simulation.

Recommended official Jira set:

```text
Use US-01 to US-15 as the official required stories.
Use US-16 as either:
- an extra story if the professor accepts 16 stories, or
- a parent task/subtask of US-10 or US-15 if you want to stay within 15 official stories.
```

Recommended subtasks for US-16 if it is not an official story:
...
US-16.6 — Select a building and view SelectedBuildingPanel
US-16.7 — Control next tick, pause, and speed from the UI
US-16.8 — Activate policies from PolicyPanel
US-16.9 — Keep HUD and renderer separated from domain logic
```
...
Do not necessarily implement the stories in numerical order. A practical order is:

```text
1. US-01 — Create a new DaimyoSimulator village
2. US-02 — Construct buildings using timber
3. US-03 — Enforce placement and prerequisite rules
4. US-04 — Manage villagers and villager roles
5. US-05 — Manage dwellings and housing status
6. US-06 — Automatically assign idle villagers to jobs
7. US-07 — Produce and consume rice
8. US-08 — Produce timber, tools, and luxury goods
9. US-10 — Advance the simulation by one tick
10. US-11 — Calculate village parameters and happiness
11. US-12 — Process births and deaths
12. US-13 — Activate one strategy policy at a time
13. US-09 — Exchange resources through markets
14. US-14 — Trigger random village events
15. US-15 — Save and load the village
16. US-16 — Interact with the village through the libGDX HUD
```

---

# Suggested design pattern mapping

| Requirement | Suggested pattern / design choice |
|---|---|
| Strategy Policies | Strategy Pattern |
| Building creation | Factory Pattern |
| libGDX Scene2D HUD updates after each tick | MVC / Observer / Clean Architecture boundary |
| Random events | Observer Pattern or event service |
| Save/load | DTO + Persistence Service |
| Tick processing | Simulation Engine coordinating smaller services |
| Placement and production rules | Rule objects / service classes |
| Testable randomness | RandomProvider interface or fixed seed |
...
To keep Jira clean, create each `US-xx` as one issue. Put the acceptance criteria in the issue description. Put the related classes and related tests in the technical notes or implementation notes field.

For system testing, each acceptance criterion can become one validation row in the system test document:

```text
Acceptance Criterion | Status OK/KO | Date | Tester | Comment
```

For unit testing, each significant service/class listed above should have a matching JUnit test class.
