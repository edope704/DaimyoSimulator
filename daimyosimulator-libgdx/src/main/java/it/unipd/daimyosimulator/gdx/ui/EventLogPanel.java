...
public final class EventLogPanel extends Table {
    ...
    public EventLogPanel(Skin skin, GameAssetManager assetManager) {
        ...
        header.add(new Image(assetManager.getParameterIcon(ParameterType.EVENT_ALERT))).size(18).padRight(5);
        header.add(new Label("EVENTS", skin, "title")).left();
        ...
        add(scroll).width(340).height(68).left().top();
    }
    ...
    private void redraw() {
        ...
        for (Entry entry : entries) {
            ...
            row.setColor(color);
            row.setWrap(true);
            rows.add(row).width(320).left().padBottom(1);
            rows.row();
        }
        rows.layout();
    }
